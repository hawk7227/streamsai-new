...
export class OpenAIGPTImage15Provider implements MediaProvider {
  readonly name = 'openai-gpt-image-1.5';
  readonly toolType = 'image' as const;

  readonly capabilities: ProviderCapabilities = {
    preview: true, final: true, batchCount: 4, webhooks: false, streaming: false,
    nativeAudio: false, maxDuration: null, maxResolution: '4096x4096',
    supportedAspectRatios: ['1:1', '16:9', '9:16', '4:3', '3:4'],
    supportedStyles: ['natural', 'vivid'],
  };

  readonly limits: ProviderLimits = { maxConcurrent: 5, requestsPerMinute: 15 };
  ...
  async generate(params: GenerationParams): Promise<GenerationResult> {
    ...
    const quality = params.quality === 'preview' ? 'low' : 'high';
    const size = params.quality === 'preview' ? '1024x1024' : '1536x1536';
    ...
    try {
      ...
      return {
        ...
        durationMs: Date.now() - start, costCents: quality === 'high' ? 25 : 2,
        metadata: { model: 'gpt-image-1.5', quality, size },
      };
    } catch (err) {
