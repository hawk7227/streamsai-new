...
export class AnthropicOpusProvider implements MediaProvider {
  readonly name = 'anthropic-opus';
  ...
  async generate(params: GenerationParams): Promise<GenerationResult> {
    ...
    const maxTokens = params.quality === 'preview' ? 1500 : 8000;
    ...
    try {
      ...
      const costCents = Math.ceil((data.usage.input_tokens * 15 + data.usage.output_tokens * 75) / 1_000_000 * 100);

      return {
        success: true, format: 'txt', durationMs: Date.now() - start, costCents: Math.max(1, costCents),
        metadata: { text, wordCount, model: 'claude-opus-4-5-20250929', tokens: data.usage },
      };
    } catch (err) {
